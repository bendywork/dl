...
# LSTM全连接文本分类任务 手写 LSTM模型下的参数配置
class LSTMClassifyConfig(object):
    def __init__(self):
        # 超参数配置 最大词表大小
...
# LSTM全连接文本分类任务手写 LSTM数据集构建
class LSTMClassifyDatasets(object):
...
# 构建词表
class LSTMClassifyBuildVocabulary(object):
...
def build_data_loader(data, vocab, config,  shuffle):
    ds = LSTMClassifyDatasets(data, vocab, config)
    return DataLoader(
        dataset = ds,
...
#     from datasets import load_dataset
#     config = LSTMClassifyConfig()
#     dataset = load_dataset(config.DATASET_NAME)
    # print(type(dataset))
    # print(dataset)
    # print("---第一条训练样本---")
    # vocab = LSTMClassifyBuildVocabulary(config, dataset['train'])
    # vocabulary = vocab.build_vocabulary()
    # my_dataset = LSTMClassifyDatasets(dataset['train'], vocabulary, config=config)
    # print(my_dataset[0])
    ...
    # model = LSTMClassifyModel(vocabulary, config)
    # batch = next(iter(data_loader))
    # out = model(batch['x'])
...
if __name__ == '__main__':
    ...
    vocab_builder = LSTMClassifyBuildVocabulary(config, dataset['train'])
    ...
    model = LSTMClassifyModel(vocabulary, config)
    trainer = LSTMClassifyTrain(config, model)
