...
# GRU全连接文本分类任务 手写 GRU模型下的参数配置
class GRUClassifyConfig(object):
    def __init__(self):
        # 超参数配置 最大词表大小
...
# GRU全连接文本分类任务手写 GRU数据集构建
class GRUClassifyDatasets(object):
...
# 构建词表
class GRUClassifyBuildVocabulary(object):
...
def build_data_loader(data, vocab, config,  shuffle):
    ds = GRUClassifyDatasets(data, vocab, config)
    return DataLoader(
        dataset = ds,
...
# GRU 文本分类模型结构：
class GRUClassifyModel(nn.Module):
...
#     from datasets import load_dataset
#     config = GRUClassifyConfig()
#     dataset = load_dataset(config.DATASET_NAME)
    # print(type(dataset))
    # print(dataset)
    # print("---第一条训练样本---")
    # vocab = GRUClassifyBuildVocabulary(config, dataset['train'])
    # vocabulary = vocab.build_vocabulary()
    # my_dataset = GRUClassifyDatasets(dataset['train'], vocabulary, config=config)
    # print(my_dataset[0])
...
if __name__ == '__main__':
    ...
    vocab_builder = GRUClassifyBuildVocabulary(config, dataset['train'])
