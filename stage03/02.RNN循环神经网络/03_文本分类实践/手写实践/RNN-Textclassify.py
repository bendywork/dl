...
# RNN全连接文本分类任务 手写 RNN模型下的参数配置
class RNNClassifyConfig(object):
    def __init__(self):
        # 超参数配置 最大词表大小
...
# RNN全连接文本分类任务手写 RNN数据集构建
class RNNClassifyDatasets(object):
...
# 构建词表
class RNNClassifyBuildVocabulary(object):
...
def build_data_loader(data, vocab, config,  shuffle):
    ds = RNNClassifyDatasets(data, vocab, config)
    return DataLoader(
        dataset = ds,
...
# RNN 文本分类模型结构：
class RNNClassifyModel(nn.Module):
    ...
    def forward(self, x):
         ...
         # rnn取最后时刻的隐藏状态[bs, t, h] -> [bs, h]
         last_hidden = torch.cat([hidden[0], hidden[1]], dim=-1)  # [bs, 512]
         # 做分类
         logits = self.classify_layer(last_hidden)
         return logits

class RNNClassifyTrain(object):
...
#     from datasets import load_dataset
#     config = RNNClassifyConfig()
#     dataset = load_dataset(config.DATASET_NAME)
    # print(type(dataset))
    # print(dataset)
    # print("---第一条训练样本---")
    # vocab = RNNClassifyBuildVocabulary(config, dataset['train'])
    # vocabulary = vocab.build_vocabulary()
    # my_dataset = RNNClassifyDatasets(dataset['train'], vocabulary, config=config)
    # print(my_dataset[0])
    ...
    # model = RNNClassifyModel(vocabulary, config)
    # batch = next(iter(data_loader))
    # out = model(batch['x'])
...
if __name__ == '__main__':
    ...
    vocab_builder = RNNClassifyBuildVocabulary(config, dataset['train'])
    ...
    trainer = RNNClassifyTrain(config, model)
