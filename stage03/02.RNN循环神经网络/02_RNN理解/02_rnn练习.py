...
def test_rnn():
    ...
    input_weight, input_bias = input_linear.weight, input_linear.bias

    # W 矩阵对应的线性层：将上一时刻隐状态 H_{t-1} 从 2e 维映射到 2e 维
    hidden_linear = nn.Linear(2 * e, e * 2, dtype=dtype)
    hidden_weight, hidden_bias = hidden_linear.weight, hidden_linear.bias
    ...
    for index in range(t):
        ...
        # U * X_t：对当前输入做线性变换
        input_transform = torch.matmul(current_input, input_weight.T) + input_bias    # [bs, 2e]

        # W * H_{t-1}：对上一时刻隐状态做线性变换
        hidden_transform = torch.matmul(h_prev, hidden_weight.T) + hidden_bias        # [bs, 2e]
        ...
        token_new_embeddings.append(h_cur[:, None])                            # [bs, 2e] --> [bs, 1, 2e]

    token_new_embeddings = torch.cat(token_new_embeddings, dim=1)  # [bs, t, 2e]
    print(token_new_embeddings.shape)
