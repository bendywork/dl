# -*- coding: utf-8 -*-
"""
GRU 门控循环单元训练模板（NLP方向 · 序列分类）
--------------------------------------------------------------
模型类型 : GRU (Gated Recurrent Unit)
适用场景 : 序列分类、语义建模（作为 LSTM 的更高效替代）
核心结构 : Embedding → GRU → Linear → 输出
相对 LSTM : 合并遗忘门和输入门为"更新门"，只有 h 没有 c，参数更少训练更快
--------------------------------------------------------------
"""
import torch
...
class GRUModel(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        output, h_n = self.gru_layer(packed)             # GRU 只有一个隐状态
        h_fwd = h_n[-2, :, :]
        h_bwd = h_n[-1, :, :]
...
def main():
    ...
    model = GRUModel(config['vocab_size'], config['embedding_dim'],
