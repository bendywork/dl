# -*- coding: utf-8 -*-
"""
LSTM 长短期记忆网络训练模板（NLP方向）
--------------------------------------------------------------
模型类型 : LSTM
适用场景 : 长序列分类、序列标注、Seq2Seq 的编码器/解码器
核心结构 : Embedding → LSTM → Linear → 输出
相对 RNN 的改进 : 引入遗忘门/输入门/输出门 + 细胞状态 C，缓解长序列梯度消失
--------------------------------------------------------------
"""
import torch
...
class LSTMModel(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        output, (h_n, c_n) = self.lstm_layer(packed)      # LSTM 返回二元组状态
        h_fwd = h_n[-2, :, :]
        h_bwd = h_n[-1, :, :]
        h_cat = torch.cat([h_fwd, h_bwd], dim=1)          # [bs, hidden*2]
        logits = self.classify_layer(h_cat)                # [bs, num_classes]
        return logits
...
def main():
    ...
    model = LSTMModel(config['vocab_size'], config['embedding_dim'],
                      config['hidden_size'], config['num_layers'], config['num_classes']).to(config['device'])
