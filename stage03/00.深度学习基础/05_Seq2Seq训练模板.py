# -*- coding: utf-8 -*-
"""
Seq2Seq 序列到序列训练模板（NLP方向 · 无 Attention 版本）
--------------------------------------------------------------
模型类型 : Seq2Seq (Encoder-Decoder with LSTM)
适用场景 : 文本翻译、文本摘要、对话系统
核心结构 : Encoder(LSTM) → 上下文向量 C → Decoder(LSTM) → 逐 token 输出
局限性   : 所有解码步共享同一个 C，长序列信息瓶颈
--------------------------------------------------------------
"""
import torch
...
class Seq2SeqModel(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        _, (h_n, c_n) = self.enc_lstm(src_embed)

        tgt_embed = self.dec_embed(tgt)
        output, _ = self.dec_lstm(tgt_embed, (h_n, c_n))
...
def main():
    ...
    model = Seq2SeqModel(config['src_vocab_size'], config['tgt_vocab_size'],
