...
@app.get("/")
def index():
    return "基于FastAPI的算法API部署后端框架 + SpERT片段网络 + ONNX"
...
def start_server(
        max_span_size=10, model_path=None, host="0.0.0.0", port=9001, size_embedding=25, rel_filter_threshold=0.3
):
    global predictor

    app.get("/predict", summary="实体识别+关系抽取")(predict)
    app.post("/predict", summary="实体识别+关系抽取")(predict)

    deploy_path = model_path or os.environ['MODEL_PATH']
    predictor = Predictor(
        model_path=deploy_path,
        tokenizer_path=deploy_path,
        types_path=os.path.join(deploy_path, "types.json"),
