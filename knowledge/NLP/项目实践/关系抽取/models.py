...
class SpERT(BertPreTrainedModel):
    ...
    def __init__(self, config: BertConfig, cls_token: int, relation_types: int, entity_types: int,
        ...
        # BERT model 基础的token特征向量提取模块
        self.bert = BertModel(config)

        # layers
        self.rel_classifier = nn.Linear(config.hidden_size * 3 + size_embedding * 2, relation_types)  # 关系分类
        self.entity_classifier = nn.Linear(config.hidden_size * 2 + size_embedding, entity_types)  # 实体分类
    ...
    def _forward_train(self, encodings: torch.tensor, context_masks: torch.tensor, entity_masks: torch.tensor,
        ...
        # classify entities 实体分类
        size_embeddings = self.size_embeddings(entity_sizes)  # embed entity candidate sizes 每个实体对应的长度特征向量
        entity_clf, entity_spans_pool = self._classify_entities(encodings, h, entity_masks, size_embeddings)

        # classify relations 关系分类
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings):
        ...
        # get cls token as candidate context representation 获取文本特征向量
    ...
    def _classify_relations(self, entity_spans, size_embeddings, relations, rel_masks, h, chunk_start):
        ...
        entity_pairs = util.batch_index(entity_spans, relations)  # 基于关系中给定的实体序号id(第几个实体)从实体特征向量中抽取出主体、客体特征向量
...
def load_model(
        model_type, model_path, relation_type_count, entity_type_count, size_embedding, cls_token_id,
        max_pairs=1000, prop_drop=0.0, freeze_transformer=True
):
    ...
    model = model_class.from_pretrained(
        ...
        relation_types=relation_type_count - 1,  # 关系的数量
        entity_types=entity_type_count,  # 实体的数量
        max_pairs=max_pairs,
        prop_drop=prop_drop,
