...
class SpERTTrainer(BaseTrainer):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        # byte-pair encoding Bert分词器的迁移恢复
        self._tokenizer = BertTokenizer.from_pretrained(
            args.tokenizer_path,
            # do_lower_case=args.lowercase,  # 数据是否转换为小写格式
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: Type[BaseInputReader]):
        ...
        input_reader = input_reader_cls(
            types_path,  # 元数据文件路径
            self._tokenizer,  # 分词器
            args.neg_entity_count,  # 实体的负标签的数目
            args.neg_relation_count,  # 关系的负标签的数目
            args.max_span_size,  # 实体最多允许有多少个token组成
            self._logger
        )
        # 加载训练数据
        ...
        train_sample_count = train_dataset.document_count  # 总训练样本数目
        updates_epoch = train_sample_count // args.train_batch_size  # 每一个epoch有多少个batch
        updates_total = updates_epoch * args.epochs  # 总的训练批次
        ...
        # create optimizer 构造优化器
        optimizer_params = self._get_optimizer_params(model)
        optimizer = AdamW(optimizer_params, lr=args.lr, weight_decay=args.weight_decay, correct_bias=False)
        # create scheduler 构造学习率变化器  --> warmup: 学习率先增大再减小这种趋势
        scheduler = transformers.get_linear_schedule_with_warmup(
            optimizer,
            num_warmup_steps=args.lr_warmup * updates_total,
            num_training_steps=updates_total  # 总的更新批次
        )
        # create loss function
        rel_criterion = torch.nn.BCEWithLogitsLoss(reduction='none')  # sigmoid交叉熵损失函数 --> 多分类 --> 一个(主体,客体)可以属于多个关系
        entity_criterion = torch.nn.CrossEntropyLoss(reduction='none')  # softmax交叉熵损失函数
        compute_loss = SpERTLoss(rel_criterion, entity_criterion, model, optimizer, scheduler, args.max_grad_norm)

        # eval validation set 判断是否需要在训练前进行评估操作
        if args.init_eval:
    ...
    def _load_model(self, input_reader):
        ...
        model = model_class.from_pretrained(
            ...
            relation_types=input_reader.relation_type_count - 1,  # 关系的数量
            entity_types=input_reader.entity_type_count,  # 实体的数量
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            # [
            #   'encodings', 'context_masks', 'entity_masks',
            #   'entity_sizes', 'entity_types', 'rels',
            #   'rel_masks', 'rel_types', 'entity_sample_masks',
            #   'rel_sample_masks'
            # ]
            # N: batch_size 批次大小； T: 序列长度； N1: 当前这个序列文本中的实体数量；N2: 当前这个序列文本中的关系数量
            # forward step 前向执行过程，结果是关系的预测置信度、实体的预测置信度
            # entity_logits: [N,N1,entity_type_num] 预测每个候选实体对应各个类别的置信度
            # rel_logits: [N,N2,relation_type_num] 预测每个候选关系对应各个类别的置信度
            entity_logits, rel_logits = model(
                encodings=batch['encodings'],  # [N,T] token id序列对象,
                context_masks=batch['context_masks'],  # [N,T] token id对应的mask信息；真实token为True，填充token为False
                entity_masks=batch['entity_masks'],  # [N,N1,T] 每个序列、每个实体对应的mask信息(每个实体的位置信息)
                entity_sizes=batch['entity_sizes'],  # [N,N1] 每个序列、每个实体的长度
                relations=batch['rels'],  # [N,N2,2] 每个序列、每个关系对应的主体索引和客体索引
                rel_masks=batch['rel_masks']  # [N,N2,T] 每个序列、每个关系中两个实体中间部分的mask信息
            )

            # compute loss and optimize parameters 计算loss损失、参数更新
    ...
    def _predict(self, model: torch.nn.Module, dataset: Dataset, input_reader: BaseInputReader):
        ...
        data_loader = DataLoader(
            dataset, batch_size=self._args.eval_batch_size, shuffle=False, drop_last=False,
            num_workers=self._args.sampling_processes, collate_fn=sampling.collate_fn_padding
        )
    ...
    def _log_train(self, optimizer: Optimizer, loss: float, epoch: int,
        ...
        if isinstance(loss, dict):
            avg_loss = loss['train_loss'] / self._args.train_batch_size
        else:
        ...
        if isinstance(loss, dict):
            self._log_csv(
                label, "all_loss",
                loss.get('train_loss', 0),
                loss.get('entity_loss', 0),
                loss.get('rel_loss', 0),
    ...
    def _init_train_logging(self, label):
        self._add_dataset_logging(label,
                                  data={'lr': ['lr', 'epoch', 'iteration', 'global_iteration'],
                                        ...
                                        'all_loss': ['train_loss', 'entity_loss', 'rel_loss', 'epoch', 'iteration',
                                                     'global_iteration'],
