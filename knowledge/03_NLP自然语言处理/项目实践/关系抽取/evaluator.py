...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: BaseInputReader, text_encoder: BertTokenizer,
        ...
        # relations 存储关系数据
        self._gt_relations = []  # ground truth 真实的关系数据存储列表
        self._pred_relations = []  # prediction 预测的关系数据存储列表

        # entities 实体存储列表
        self._gt_entities = []  # ground truth 真实的实体数据存储列表
        self._pred_entities = []  # prediction 预测的实体数据存储列表
