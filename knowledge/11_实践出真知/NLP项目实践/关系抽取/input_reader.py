...
class JsonInputReader(BaseInputReader):
    ...
    def read(self, dataset_path, dataset_label):
        dataset = Dataset(
            dataset_label,  # 当前ds的类型标签字符串
            self._relation_types,  # 关系名称到class对象的映射mapping
            self._entity_types,  # 实体名称到class对象的映射mapping
            self._neg_entity_count,  # 负实体标签的数量
            self._neg_rel_count,  # 负关系标签的数量
            self._max_span_size  # 实体最多允许有多少个token组成
        )
        self._parse_dataset(dataset_path, dataset)  # 解析dataset数据 --> 分词、token和entity、rel之间的映射
        self._datasets[dataset_label] = dataset  # 将dataset存储下来
        return dataset

    def _parse_dataset(self, dataset_path, dataset):
        documents = json.load(open(dataset_path, "r", encoding="utf-8"))
        for document in tqdm(documents, desc="Parse dataset '%s'" % dataset.label):
            self._parse_document(document, dataset)  # 处理当前文本

    def _parse_document(self, doc, dataset) -> Document:
        jtokens = doc['tokens']  # tokens列表
        jrelations = doc['relations']  # 关系列表
        jentities = doc['entities']  # 实体列表

        # parse tokens 将token转换为token id
        doc_tokens, doc_encoding = _parse_tokens(jtokens, dataset, self._tokenizer)

        # parse entity mentions 实体解析
        entities = self._parse_entities(jentities, doc_tokens, dataset)

        # parse relations 关系解析
        relations = self._parse_relations(jrelations, entities, dataset)

        # create document 创建样本，包含：tokens、输入模型的token ids、实体信息、关系信息
    ...
    def _parse_entities(self, jentities, doc_tokens, dataset) -> List[Entity]:
        ...
        for entity_idx, jentity in enumerate(jentities):
            entity_type = self._entity_types[jentity['type']]  # 当前实体对应的类型
            start, end = jentity['start'], jentity['end']  # 当前实体在原始tokens序列中的起始、终止的位置
