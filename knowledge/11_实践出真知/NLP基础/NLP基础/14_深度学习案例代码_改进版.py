...
class NumpyDataset(Dataset):
    ...
    def __getitem__(self, item):
        _x = torch.tensor(self.x[item], dtype=torch.float32)
        _y = torch.tensor(self.y[item], dtype=torch.int64)
        return {
            "x": _x,
            "y": _y
        }
...
def build_numpy_dataset(x, y, batch_size, shuffle):
    ds = NumpyDataset(x, y)
    return DataLoader(
        dataset=ds,
...
def training():
    ...
    train_dataloader = build_numpy_dataset(x_train, y_train, batch_size, True)
    test_dataloader = build_numpy_dataset(x_test, y_test, batch_size * 2, False)
    ...
    for epoch in range(total_epoch):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            # 获取当前批次的数据x + y
            batch_x_train = batch['x']
            batch_y_train = batch['y']
            ...
            print(f"Train Epoch {epoch}/{total_epoch} Batch {batch_idx} Loss:{loss.item():.3f}")
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(test_dataloader):
                # 获取当前批次的数据x + y
                batch_x_test = batch['x']
                batch_y_test = batch['y']
                ...
                print(f"Test Epoch {epoch}/{total_epoch} Batch {batch_idx} "
