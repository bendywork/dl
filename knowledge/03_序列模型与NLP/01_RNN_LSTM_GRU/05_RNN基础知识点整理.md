...
### 知识点7：为什么 RNN 隐藏状态用 H 命名

**H 是 Hidden State（隐藏状态）的首字母缩写。**

#### 为什么叫"隐藏"？

因为 h_t 不是直接对外输出的结果，而是网络内部自己维护的中间状态：

```
输入层 x_t → [隐藏层 h_t] → 输出层 y_t
                  ↑
             外部看不到这里
             这是网络的"内部记忆"
```

你能直接看到的：输入 x_t、输出 y_t  
你看不到的：h_t（网络内部在每一步"记住了什么"）

所以叫 Hidden，隐藏的。

#### h_t 在 RNN 里具体代表什么？

**h_t 是网络在读到第 t 个 token 之后，对"目前为止整个序列"的压缩摘要。**

```
读完"我"         → h_1 = 网络对"我"的理解
读完"我喜欢"     → h_2 = 网络对"我喜欢"的理解
读完"我喜欢深度" → h_3 = 网络对"我喜欢深度"的理解
```

它不是某一个词的信息，而是**截止到当前时刻，整个历史的浓缩**。
...
|------|------|------|
| x | input | 输入 |
| **h** | **hidden state** | **隐藏状态（内部记忆）** |
| y | output | 输出 |
| W | weight | 权重矩阵 |
| b | bias | 偏置 |

普通前馈神经网络的中间层也叫 hidden layer，激活值也用 h 表示。RNN 沿用了这个传统，增加了时间下标 t，表示它随时间变化。

**一句话总结：h_t = 网络在 t 时刻的"工作记忆"，Hidden 的意思是它是网络内部状态，不直接对外暴露。**
