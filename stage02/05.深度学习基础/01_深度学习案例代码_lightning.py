...
def training():
    ...
    os.makedirs("output/02/lightning", exist_ok=True)
    trainer = pl.Trainer(
        max_epochs=100,
        default_root_dir="output/02/lightning",
        enable_progress_bar=True,
    )
    trainer.fit(net, train_loader, test_loader)

    # 5. 模型保存（与原文件 torch.save 对应）
    torch.save({'net': net, 'net_param': net.state_dict()}, "output/02/lightning/final.pkl")
    print("训练完成，模型已保存")


def interface():
    obj = torch.load("output/02/lightning/final.pkl", map_location='cpu', weights_only=False)
    net = obj['net']
    net.eval()
